...
options:
  ...
  resource:
    ...
    default: null
    description:
      - The resource to perform an action on. pods (po),
  ...
  all:
    ...
    default: false
    description:
      - A flag to indicate delete all, stop all,
        or all namespaces when checking exists.
  log_level:
    required: false
    default: 0
    description:
  ...
  state:
    ...
    default: present
    description:
      - present: checking existence or creating if definition file provided,
        absent: deleting resource(s) based on other options,
        latest: creating or updating based on existence,
        reloaded: updating resource(s) definition using definition file,
        stopped: stopping resource(s) based on other options.
        labeled: labelling resource(s), overwriting labels with same name
requirements:
  - kubectl
...
class KubeManager(object):
    ...
    def __init__(self, module):
        ...
        if module.params.get('log_level'):
            self.base_cmd.append(
                '--v=' + str(module.params.get('log_level')))
        ...
        try:
            ...
            if rc != 0:
                self.module.fail_json(
                    msg='error running kubectl ({}) command (rc={}): {}'.format(
                        ' '.join(args), rc, out or err))
        except Exception as exc:
            self.module.fail_json(
                msg='error running kubectl ({}) command: {}'.format(
                    ' '.join(args), str(exc)))
    ...
    def stop(self):
        ...
        if self.filename:
            ...
            if not self.resource:
                self.module.fail_json(
                    msg='resource required to stop without filename')
    ...
    def ensure_label(self):
        cmd = ['label', self.resource, '--overwrite', self.name, self.label]
        return self._execute(cmd)
